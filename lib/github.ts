...
const FALLBACK: Issue[] = [
  ...
  {
    number: 62,
    title: "Crabionics Foundation Baseline — cross-repository build programme",
    state: "open",
    labels: ["Foundation", "PMO"],
    url: "https://github.com/Crabionics/crabionics-pmo/issues/62",
    updatedAt: "",
  },
];
...
export function filterStateForRole(state: ControlTowerState, role: Role) {
  ...
  const blockers = state.pmoIssues.filter((issue) =>
  ...
  return {
    ...
    pmoIssues: permissions.links ? state.pmoIssues : state.pmoIssues.map(({ url: _url, ...issue }) => issue),

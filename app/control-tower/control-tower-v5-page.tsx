import ControlTowerV5 from "./control-tower-v5";

type TowerData = Parameters<typeof ControlTowerV5>[0]["data"];
...
export default async function ControlTowerV5Page() {
  const pmoState = await readPmo(STATE_PATH);
  const data: TowerData = { live: Boolean(pmoState), githubLive: Boolean(pmoState), pmoState, pmoStateUrl: STATE_URL, validation: "", technology: "" };
  return <ControlTowerV5 data={data} />;
}
